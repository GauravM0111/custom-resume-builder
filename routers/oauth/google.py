...
async def sign_in(guest_id: Optional[str] = None):
    ...
    response = RedirectResponse(
        url="{}?{}".format(
            retrieve_discovery_document()["authorization_endpoint"], urlencode(params)
        )
    )
...
async def callback(
    ...
    guest_id: Annotated[str | None, Cookie()] = None,
    ...
    try:
        ...
        try:
            if guest_id:
                user = UserService().create_user_from_guest(
                    guest_id, UserCreate(**response.json()), db
                )
            else:
